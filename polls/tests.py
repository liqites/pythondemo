...
def create_question(question_text, days):
    """
    Creates a question with the given `question_text` and published the given of `days` offset to now (negative for questions published in the past, positive for questions that have yet to be published)
    """
...
class QuestionMethodTests(TestCase):
    ...
    def test_was_published_recently_with_old_question(self):
        """
        was_published_recently() should return False for question whose pug_date is orlder than 1 day.
        """
        time = timezone.now() - datetime.timedelta(days=30)
